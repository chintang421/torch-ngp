...
class TNeRFRenderer(nn.Module):
    ...
    def run(self, rays_o, rays_d, time, num_steps=128, upsample_steps=128, bg_color=None, perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), time.repeat(xyzs.reshape(-1, 3).shape[0],1))
        ...
        if upsample_steps > 0:
            ...
            # only forward new points to save computation
            new_density_outputs = self.density(new_xyzs.reshape(-1, 3), time.repeat(new_xyzs.reshape(-1, 3).shape[0],1))
            #new_sigmas = new_density_outputs['sigma'].view(N, upsample_steps) # [N, t]
            for k, v in new_density_outputs.items():
                new_density_outputs[k] = v.view(N, upsample_steps, -1)
    ...
    def run_cuda(self, rays_o, rays_d, time, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, **kwargs):
        ...
        if self.training:
            ...
            density_outputs = self.density(xyzs, time.repeat(xyzs.shape[0],1)) # [M,], use a dict since it may include extra things, like geo_feat for rgb.
            sigmas = density_outputs['sigma']
            sigmas = self.density_scale * sigmas
            rgbs = self.color(xyzs, dirs, **density_outputs)
            ...
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    for cas in range(self.cascade):
                        ...
                        cas_xyzs += (torch.rand_like(cas_xyzs) * 2 - 1) * half_grid_size
                        # query density
                        sigmas = self.density(cas_xyzs.to(tmp_grid.device), torch.zeros(cas_xyzs.shape[0], 1).to(tmp_grid.device))['sigma'].reshape(lx, ly, lz).detach() ##gg??
                        # from `scalbnf(MIN_CONE_STEPSIZE(), 0)`, check `splat_grid_samples_nerf_max_nearest_neighbor`
                        # scale == 2 * sqrt(3) / 1024
                        sigmas *= self.density_scale * 0.003383
                        # assign 
    ...
    def render(self, rays_o, rays_d, times, staged=False, max_ray_batch=4096, bg_color=None, perturb=False, **kwargs):
        # rays_o, rays_d: [B, N, 3], assumes B == 1
        # times: [B], assumes B == 1
        # return: pred_rgb: [B, N, 3]
        ...
        if staged and not self.cuda_ray:
            ...
            for b in range(B):
                ...
                while head < N:
                    tail = min(head + max_ray_batch, N)
                    results_ = _run(rays_o[b:b+1, head:tail], rays_d[b:b+1, head:tail], times[b:b+1], bg_color=bg_color, perturb=perturb, **kwargs)
                    depth[b:b+1, head:tail] = results_['depth']
                    image[b:b+1, head:tail] = results_['image']
                    head += max_ray_batch
            ...
            results = _run(rays_o, rays_d, times, bg_color=bg_color, perturb=perturb, **kwargs)
